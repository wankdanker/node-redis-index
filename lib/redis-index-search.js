...
RedisIndexSearch.prototype.exec = function (cb) {
	...
	self.searches.forEach(function (search, i) {
		...
		Object.keys(obj).forEach(function (key) {
			...
			if (val instanceof RedisIndexSearchComparison) {
				if (val.type == 'gte') {
					ssk2 = k('z', 'search-intermediary', intermediaries.length);
					commands.push(['eval', lua.zrangebyscorestore, 2, ssk2, k('z', key), val.a, '+inf']);
					keys.push(ssk2);
				...
				if (val.type == 'lte') {
					ssk2 = k('z', 'search-intermediary', intermediaries.length);
					commands.push(['eval', lua.zrangebyscorestore, 2, ssk2, k('z', key), '-inf', val.a]);
					keys.push(ssk2);
					intermediaries.push(ssk2);
					offset += 1;
				...
				if (val.type == 'between') {
					ssk2 = k('z', 'search-intermediary', intermediaries.length);
					commands.push(['eval', lua.zrangebyscorestore, 2, ssk2, k('z', key), val.a, val.b]);
					keys.push(ssk2);
					intermediaries.push(ssk2);
					offset += 1;
