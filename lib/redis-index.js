...
comparisons.copyTo(RedisIndex);

module.exports.obtainClient = function (opts) {
	return redis.createClient(opts);
}

module.exports.releaseClient = function (err, client) {
	client.quit();
}
 ...
 * @param  {Object} [opts.compress] And object that provides syncrhonous encode and decode functions
 * @param  {Object} [opts.compress.encode] Function called to compress the value of a stored key
 * @param  {Object} [opts.compress.decode] Function called to decmpress the value of a stored key upon retrieval
 * @param  {Function} [opts.obtainClient] optional function that should return a new redis client
 * @param  {Function} [opts.releaseClient] optional function that should handle the closing of a redis client
...
function RedisIndex (opts) {
	...
	self.client = null
	self.obtainClient = opts.createClient || opts.obtainClient || module.exports.createClient || module.exports.obtainClient;
	self.releaseClient= opts.releaseClient || module.exports.releaseClient;
	self.compress = opts.compress;

	comparisons.copyTo(self);
}
...
RedisIndex.prototype.add = function (obj, cb) {
	...
	if (self.storeObject) {
		...
		if (self.compress) {
			val = self.compress.encode(val);
		}

		commands.push(['set', ok, val]);
	}
	...
	function indexObject(rootSchema, rootKey, obj) {
		...
		Object.keys(obj).forEach(function (key) {
			...
			if (val == null || val == undefined) {
				if (schema.indexNull) {
					return indexNull(schema, indexKey);
				}
	...
	function indexNull (schema, key) {
		debug('indexNull: %s', key, schema);
 ...
 */
RedisIndex.prototype.search = function (obj, type, cb) {
	var self = this;
