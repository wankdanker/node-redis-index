...
RedisIndex.prototype.add = function (obj, cb) {
	...
	Object.keys(obj).forEach(function (key) {
		...
		if (isString(schema, val)) {
			return indexString(schema, key, val);
		}

		//handle dates
		if (isDate(schema, val)) {
			return indexDate(schema, key, val);
		}

		//handle numbers
		if (isNumber(schema, val)) {
			return indexNumber(schema, key, val);
		}

		// handle everything else
		return indexOther(schema, key, val);
	});

	//handle keys that are specified by the schema
	each(self.schema, function (key, schema) {
		var val = dv(obj, key);
